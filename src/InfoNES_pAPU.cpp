...
int ApuWriteMmc5Wave1( int cycles, int event )
{
    while ( ( event < cur_event ) && ( ApuEventQueue[event].time < cycles ) )
    {
...
void ApuRenderingMmc5Pulse1( void )
{
  int cycles = 0;
  int event = 0;
  ...
  {
    cycles += ApuCyclesPerSample;
    event = ApuWriteMmc5Wave1( cycles, event );

    /* Envelope decay at 240 Hz */
    ApuMmc5P1EnvPhase -= 4;
    ...
    /* No sweep for MMC5 */
    /* No frequency limit for MMC5 (unlike APU, freqs < 8 still play) */
    ...
    if ( ( ApuMmc5CtrlNew & 0x01 ) && ( ApuMmc5P1Atl || ApuMmc5P1Hold ) )
    {
      ...
      if ( ApuMmc5P1Env )
      {
        mmc5_wave_buffers[0][i] = ApuMmc5P1Wave[ApuMmc5P1Index >> 24] * ( ApuMmc5P1Vol + ApuMmc5P1EnvVol );
      } else {
        mmc5_wave_buffers[0][i] = ApuMmc5P1Wave[ApuMmc5P1Index >> 24] * ApuMmc5P1Vol;
      }
    } else {
      mmc5_wave_buffers[0][i] = 0;
    }
  }
  /* Length counter at 240Hz (2x APU rate) */
...
int ApuWriteMmc5Wave2( int cycles, int event )
{
    while ( ( event < cur_event ) && ( ApuEventQueue[event].time < cycles ) )
    {
...
void ApuRenderingMmc5Pulse2( void )
{
  int cycles = 0;
  int event = 0;
  ...
  {
    cycles += ApuCyclesPerSample;
    event = ApuWriteMmc5Wave2( cycles, event );
    ...
    if ( ( ApuMmc5CtrlNew & 0x02 ) && ( ApuMmc5P2Atl || ApuMmc5P2Hold ) )
    {
      ...
      if ( ApuMmc5P2Env )
      {
        mmc5_wave_buffers[1][i] = ApuMmc5P2Wave[ApuMmc5P2Index >> 24] * ( ApuMmc5P2Vol + ApuMmc5P2EnvVol );
      } else {
        mmc5_wave_buffers[1][i] = ApuMmc5P2Wave[ApuMmc5P2Index >> 24] * ApuMmc5P2Vol;
      }
    } else {
      mmc5_wave_buffers[1][i] = 0;
    }
  }
  /* Length counter at 240Hz (2x APU rate) */
...
int ApuWriteVrc6Wave1( int cycles, int event )
{
    while ( ( event < cur_event ) && ( ApuEventQueue[event].time < cycles ) )
    {
      ...
      else if ( ApuEventQueue[event].type == APUET_W_VRC6_FREQ )
      {
        ...
        /* Recompute skip with new freq scaling */
        DWORD freq = ( ( (DWORD)( ApuVrc6P1c & 0x0F ) << 8 ) | ApuVrc6P1b );
        if ( ApuVrc6FreqCtrl & 0x04 ) freq >>= 8;
        else if ( ApuVrc6FreqCtrl & 0x02 ) freq >>= 4;
        ApuVrc6P1Skip = freq ? ( ApuPulseMagic << 1 ) / freq : 0;
...
void ApuRenderingVrc6Pulse1( void )
{
  int cycles = 0;
  int event = 0;
  ...
  {
    cycles += ApuCyclesPerSample;
    event = ApuWriteVrc6Wave1( cycles, event );

    /* Halted by $9003 bit 0 */
    if ( ApuVrc6FreqCtrl & 0x01 )
    {
      /* Hold current state - don't advance, keep last output */
      continue;
    }
...
int ApuWriteVrc6Wave2( int cycles, int event )
{
    while ( ( event < cur_event ) && ( ApuEventQueue[event].time < cycles ) )
    {
...
void ApuRenderingVrc6Pulse2( void )
{
  int cycles = 0;
  int event = 0;
  ...
  {
    cycles += ApuCyclesPerSample;
    event = ApuWriteVrc6Wave2( cycles, event );
...
int ApuWriteVrc6SawWave( int cycles, int event )
{
    while ( ( event < cur_event ) && ( ApuEventQueue[event].time < cycles ) )
    {
...
void ApuRenderingVrc6Saw( void )
{
  ...
  {
    cycles += ApuCyclesPerSample;
    event = ApuWriteVrc6SawWave( cycles, event );
...
void InfoNES_pAPUVsync(void)
{
  ...
  if ( ApuMmc5Enable )
  {
    ApuRenderingMmc5Pulse1();
    ApuRenderingMmc5Pulse2();
    /* MMC5 PCM: fill buffer with current PCM value */
    for ( unsigned int i = 0; i < ApuSamplesPerSync; i++ )
    {
  ...
  if ( ApuVrc6Enable )
  {
    ApuRenderingVrc6Pulse1();
    ApuRenderingVrc6Pulse2();
    ApuRenderingVrc6Saw();

    /* Mix VRC6 expansion audio into APU wave buffers */
    for ( unsigned int i = 0; i < ApuSamplesPerSync; i++ )
    {
